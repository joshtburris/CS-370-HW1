...
{
    // Generate a random number. Map it to the range [100000 - 120000) using the utility function 'get_iteration_count'.
    int number_of_iterations = get_iteration_count(rand());
    printf("[MemoryManager] Number of Iterations: %d\n", number_of_iterations);
    
    int divisible_medians = 0;
    
    for (int i = 0; i < number_of_iterations; ++i) {
    
		// Generate an array size by generating a random number and mapping it to the range [1000 - 1500) using the utility function 'get_arr_size'.
		...
		// Get the median of the array using the function 'return_median'.
